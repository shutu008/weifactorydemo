...
public class ServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception{
        ...
        if (flag){
            ...
            NettyChannelMap.add(request, (SocketChannel)ctx.channel());
            //客户端连接
            CommunicationLog c = getCommunicationLog(ctx, OptionType.CONNECT.getValue(), request);
            communicationService.save(c);

            LOGGER.info("设备序列号已经与设备绑定,设备序列号："+request+";Channel对象："+(SocketChannel)ctx.channel());
            Integer status = newConnection.connStatus(request);
